      ...
      <div className="border-b border-gray-200">
        <nav className="flex gap-6 -mb-px">
          {[
            ...
            { id: 'timelogs', label: 'Time Logs' },            { id: 'payout', label: 'Payout Setup' },          ].map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
...
function PayoutSetupPanel({ profile }) {
  const createAccount = useCreateTalentStripeAccount();
  const statusQuery = useTalentStripeAccountStatus(profile?.id);
