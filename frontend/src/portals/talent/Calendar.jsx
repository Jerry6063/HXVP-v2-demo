...
export default function TalentCalendar() {
  ...
  // Booked dates (set of dateStr)
  const bookedDates = useMemo(() => {
    const set = new Set();
    bookings.forEach((b) => {
      if (b.shoot_detail?.shoot_date && b.status === 'accepted') {
        set.add(b.shoot_detail.shoot_date);
      }
    });
    return set;
  }, [bookings]);
  ...
  const selectRange = useCallback((from, to) => {
    setSelectedDays((prev) => {
      ...
      dayRange(from, to).forEach((d) => {
        ...
        if (!bookedDates.has(dateStr)) next.add(d);
      });
      return next;
    });
  }, [viewYear, viewMonth, bookedDates]);
  ...
  }, [viewYear, viewMonth, bookedDates, toggleDay, selectRange]);
  ...
  }, [viewYear, viewMonth, bookedDates, toggleDay]);
    ...
    if (day != null && touchStart.current) {
      setSelectedDays(() => {
        ...
        dayRange(touchStart.current.day, day).forEach((d) => {
          ...
          if (!bookedDates.has(ds)) next.add(d);
        });
        return next;
      });
    }
  }, [viewYear, viewMonth, bookedDates, getTouchDay]);
  ...
  const selectWeekdays = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dateStr = fmtDate(viewYear, viewMonth, d);
      if (dow !== 0 && dow !== 6 && !bookedDates.has(dateStr)) next.add(d);
    }
  ...
  const selectAll = () => {
    const next = new Set();
    for (let d = 1; d <= daysInMonth; d++) {
      if (!bookedDates.has(fmtDate(viewYear, viewMonth, d))) next.add(d);
    }
    setSelectedDays(next);
  };
            ...
            return (
              <div key={wi} className="grid grid-cols-[auto_repeat(7,1fr)] gap-1 mb-1">
                ...
                <button
                  ...
                  className="w-6 flex items-center justify-center text-gray-300 hover:text-amber-500 hover:bg-amber-50 rounded text-xs transition"
                ...
                {week.map((day, di) => {
                  ...
                  return (
                    <div
                      ...
                      } ${isBooked ? 'cursor-default' : 'hover:brightness-95'}`}
                      style={{ minHeight: '64px' }}
                    >
                      {/* Date number */}
                      <span className={`absolute top-1.5 left-2 text-xs font-bold ${isToday ? 'text-amber-700' : textColor}`}>
                        {day}
                      </span>
                      ...
                      {info?.mixed && !isSelected && !isBooked && (
                      ...
                      {info && !info.mixed && !isSelected && !isBooked && (
                ...
                {bookings
                  ...
                  .map((b) => (
                    <div key={b.id} className="p-2 bg-blue-50 rounded-lg text-xs">
                      <p className="font-medium text-blue-900">{b.shoot_detail?.shoot_date}</p>
                      <p className="text-blue-700 mt-0.5">{b.shoot_detail?.location || 'Location TBD'}</p>
                    </div>
                  ))}
              </div>
            )}
